...
export default APP.manualesAdmin = (function() {
	let init = () => {
		menuOptionClick();
		//Archivos();
		llenarTabla();
	...
	let llenarTabla = () => {
		...
		$("#DataTables").DataTable({
			...
			processing: true,
			ajax: {
				url: base_url + "/ManualesController/listarArchivos/administrativo",
				dataSrc: "",
		...
		$("#DataTables tbody").on("click", "button", function() {
			...
			if (action == "descargar")
				window.location.href = `${base_url}/manuales_adm_download/${data.manuales}`;
	...
	let eliminarArchivo = archivo => {
		$.ajax({
			type: "POST",
			dataType: "text",
			url: `${base_url}/manuales_adm_remove/${archivo.manuales}`,
